...
*            par1 = m = multiplier
*
.subckt ndhvta d g s b sub w=1e-6 l=1e-6 ad=-1 as=-1 pd=-1 ps=-1 nrd=-1 nrs=-1 
+ par1=1 ldrift=0 coselfheat=1 xf_subext=0 extlay=0
xocc_m1 d g s b sub  ndhvta_ahdl tstep_occ=tstep_soac maxEv=maxEv maxEv_flg=maxEv_flg errFileName=str(errFileName)
...
.endif
m1 d g s b1 sub ndhvtamod w='w+1e-6' l='l+0.55e-6'
+ ad=0 as=0 pd=0 ps=0 nrd=0 nrs=0
+ coselfheat=coselfheat
.model ndhvtamod nmos
